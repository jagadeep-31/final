...
with st.expander("Select Project to Add Task", expanded=True):
    ...
    filtered_project_options = {
        k: v for k, v in project_options.items() if k not in ongoing_project_names
    }

    sel_project_name = st.selectbox(
        "Select Existing Project",
        list(filtered_project_options.keys())
    )
    ...
    with st.form("add_task_existing_project"):
        ...
        if submitted_existing:
            ...
            stage_id = next(
                (stage['id'] for stage in project_stages if stage['name'] == kanban_stage),
                None
            )
